...
The staged and post-promotion smoke each send a deterministic credential that cannot be generated by the
application to the normal session-bootstrap endpoint. The expected `401` is returned only after the deployed
authentication path hashes the credential and performs its normal session lookup through the pooled runtime
`DATABASE_URL`. This proves a least-privilege application-role read against the migrated `users` and
`user_sessions` schema without creating or modifying a user, session, form, submission, or seed record. The
credential and response body are not emitted; failure exposes no database identity or connection detail.
